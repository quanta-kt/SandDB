...
impl FsSSTReader {
    ...
    pub fn cached<'a>(self) -> CachedSSTableReader<'a, Self> {
...
pub struct CachedSSTableReader<'l, S: SSTableReader> {
    chunk_desc_cache: LruCache<'l, String, Vec<ChunkDesc>>,
    chunk_cache: LruCache<'l, (u64, usize), BTreeMap<String, Vec<u8>>>,
    source: S,
}

impl<'a, S: SSTableReader> CachedSSTableReader<'a, S> {
...
impl<'l, S: SSTableReader> SSTableReader for CachedSSTableReader<'l, S> {
    ...
    F: Read + Seek,
{
    ...
    fn read_footer(&mut self) -> Footer {
        self.file.seek(SeekFrom::End(-20)).unwrap();
