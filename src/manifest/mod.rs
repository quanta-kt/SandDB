...
impl ManifestWriter {
    ...
    fn compact(&mut self) {
        self.file.seek(SeekFrom::Start(0)).unwrap();
        let manifest = ManifestReader::new(&mut self.file).read_skip_invalid().unwrap();
        ...
        for sstable in manifest.sstables {
            txn.write_sstable_with_id(sstable.level, &sstable.min_key, &sstable.max_key, sstable.id);
        }
...
mod tests {
    ...
    fn test_manifest_does_not_persist_until_commit() {
        ...
        let mut writer =
            ManifestWriter::open(PathBuf::from(filename))
                .unwrap();
