...
use crate::{
    manifest::{Manifest, ManifestReader, ManifestWriter, SSTable}, sstable::{
        reader::{CachedSSTableReader, FsSSTReader, SSTableReader}, SSTableWriter
    }, Store
};
...
impl LSMTree<CachedSSTableReader<'_, FsSSTReader>> {
...
impl<S: SSTableReader> LSMTree<S> {
    ...
    fn merge_ssts(&mut self, to_merge: Vec<SSTable>, target_level: u8) -> io::Result<()> {
        ...
        let writer = SSTableWriter::new(File::create(
            sst_file_path(&self.directory, sst_id),
        )?);
