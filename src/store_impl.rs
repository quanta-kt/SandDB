...
impl StoreImpl<LSMTree<CachedSSTableReader<'_, FsSSTReader>>> {
    pub fn open(directory: PathBuf) -> io::Result<StoreImpl<LSMTree<CachedSSTableReader<'static, FsSSTReader>>>> {
...
impl<L: Store> Store for StoreImpl<L> {
    fn insert(&mut self, key: &str, value: &[u8]) -> io::Result<()> {
        ...
        if value.len() > MAX_SIZE {
            return Err(io::Error::new(io::ErrorKind::InvalidInput, "Value too long"));
        }
